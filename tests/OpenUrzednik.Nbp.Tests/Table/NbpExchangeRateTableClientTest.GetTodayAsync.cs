...
public partial class NbpExchangeRateTableClientTest
{
    ...
    public async Task GetTodayAsync_InvalidTable_ReturnsFailureWithoutSendingRequest(TableType table)
    {
        // Arrange
    ...
    public async Task GetTodayAsync_SuccessfulResponse_ReturnsMappedExchangeRateTableFromFirstArrayElement(TableType table)
    {
        // Arrange
        var faker = new Faker().WithConstantSeed();
    ...
    public async Task GetTodayAsync_HttpRequestFails_ReturnsFailureWithoutAttemptingMapping()
    {
        ...
        // Act
        var result = await sut.GetTodayAsync(TableType.A, TestContext.Current.CancellationToken);
